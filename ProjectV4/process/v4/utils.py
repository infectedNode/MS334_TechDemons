...
def draw_outputs(img, outputs, class_names):
    ...
    font = ImageFont.truetype(font='./process/data/fonts/futur.ttf',size=(15))
    count = 0

    for i in range(nums):
        temp_class = class_names[int(classes[i])]
        if (temp_class=="suitcase" or temp_class=="handbag" or temp_class=="backpack"):
            ...
            text = 'ID: {} - {}'.format(count, confidence)
            text_size = draw.textsize(text, font=font)
            draw.rectangle([x0, y0, x0 + text_size[0], y0 + text_size[1]], fill=tuple(color))
